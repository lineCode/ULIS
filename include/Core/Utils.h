...
template< typename T > constexpr ULIS3_FORCEINLINE ufloat _TypeToFloat( const uint8* iSrc, uint8 iIndex ) { return  ConvType< T, ufloat >( *( ( (T*)( iSrc ) ) + iIndex ) ); }
template< typename T > constexpr ULIS3_FORCEINLINE void _FloatToType( uint8* iSrc, uint8 iIndex, ufloat iValue ) { *( ( (T*)( iSrc ) ) + iIndex ) = ConvType< ufloat, T >( iValue ); }
