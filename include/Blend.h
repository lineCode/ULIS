...
/// @fn         Blend( const FBlock* iSource, FBlock* iBackdrop, eBlendingMode iMode, const FRect& iArea, float iOpacity = 1.f, const FPerf& iPerf = FPerf(), bool iCallInvalidCB = true )
...
/// @details    The block on top will be blended on the back block, according to the specified parameters.
/// @param      iSource         The pointer to the \e IBlock on top ( remains untouched ).
/// @param      iBackdrop       The pointer to the \e IBlock on back ( receives the blend ).
