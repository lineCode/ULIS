...
struct TSpecReg
{
    ...
    template< int N, typename T, T... Nums > constexpr const TSpecReg< N > push_back_impl( uint32 i, ::ULIS::_CT::integer_sequence< T, Nums... > ) const { return { n[Nums] ..., i }; }
    constexpr const TSpecReg< S + 1 > push_back( uint32 i ) const { return push_back_impl< S + 1 >( i, ::ULIS::_CT::make_integer_sequence< int, S >() ); }
};
