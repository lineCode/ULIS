...
template< typename T >
void
InvokeBlendMTProcessScanline_NonSeparable_CM_RGB_MEM_Generic_Subpixel( int32            iLINE
                                                                     , const tByte*     iSRC
                                                                     , tByte*           iBDP
                                                                     , uint8*           iXIDT
                                                                     , uint8            iBPC
...
template< typename T >
void
BlendMT_NonSeparable_CM_RGB_MEM_Generic_Subpixel( const FFormatInfo& iFormatInfo, const FPerfInfo& iPerfParams, std::shared_ptr< const FBlendInfo > iBlendParams ) {
    /*
    uint8* xidt;
    ...
    ULIS2_MACRO_INLINE_PARALLEL_FOR( iPerf, iPool, iBlocking, iBdpROI.h, InvokeBlendMTProcessScanline_NonSeparable_CM_RGB_MEM_Generic_Subpixel< T >, pLINE
...
template< typename T >
void
InvokeBlendMTProcessScanline_NonSeparable_CM_RGB_MEM_Generic( int32             iLINE
                                                            , const tByte*      iSRC
                                                            , tByte*            iBDP
                                                            , uint8*            iXIDT
                                                            , uint8             iBPC
                                                            ...
                                                            , ufloat            iOpacity )
{
    ...
    for( int x = 0; x < iBdpROI.w; ++x ) {
        ...
        FRGBF src_rgbf = { TYPE2FLOAT( src, iXIDT[0] ), TYPE2FLOAT( src, iXIDT[1] ), TYPE2FLOAT( src, iXIDT[2] ) };
        FRGBF bdp_rgbf = { TYPE2FLOAT( bdp, iXIDT[0] ), TYPE2FLOAT( bdp, iXIDT[1] ), TYPE2FLOAT( bdp, iXIDT[2] ) };
        FRGBF result_rgbf;
        #define TMP_ASSIGN( _BM, _E1, _E2, _E3 ) result_rgbf = NonSeparableOpF< _BM >( src_rgbf, bdp_rgbf );
        ULIS2_SWITCH_FOR_ALL_DO( iBlendingMode, ULIS2_FOR_ALL_NONSEPARABLE_BM_DO, TMP_ASSIGN, 0, 0, 0 )
...
template< typename T >
void
BlendMT_NonSeparable_CM_RGB_MEM_Generic( const FFormatInfo& iFormatInfo, const FPerfInfo& iPerfParams, std::shared_ptr< const FBlendInfo > iBlendParams ) {
/*
    uint8* xidt;
    uint8 bpc, ncc, hea, spp, bpp, aid;
    tSize roi_w, roi_h, src_bps, bdp_bps, src_jmp, bdp_jmp;
    ...
    const tByte*    src = iSource->DataPtr();
    tByte*          bdp = iBackdrop->DataPtr();
    ULIS2_MACRO_INLINE_PARALLEL_FOR( iPerf, iPool, iBlocking, iBdpROI.h, InvokeBlendMTProcessScanline_NonSeparable_CM_RGB_MEM_Generic< T >, pLINE
                                                                       , src + ( ( iSrcROI.y + pLINE ) * src_bps ) + ( iSrcROI.x * bpp )
                                                                       , bdp + ( ( iBdpROI.y + pLINE ) * bdp_bps ) + ( iBdpROI.x * bpp )
                                                                       , xidt, bpc, ncc, hea, spp, bpp, aid, src_bps, iSrcROI, iBdpROI
                                                                       , iSubpixelComponent, iBlendingMode, iAlphaMode, iOpacity );
