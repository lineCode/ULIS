    ...
    , mLeftButtonDown( false )
{
    ...
    mParticle = new FBlock( 1, 1, ULIS2_FORMAT_RGBA8 );
...
SWindow::tickEvent() {
    ...
    if( mLeftButtonDown ) {
        for( int i = 0; i < 200; ++i ) {
            float dir = ( rand() % 100 ) / 50.f * 3.14159265359;
            float vel = ( rand() % 100 ) / 200.f + 1;
    ...
    for( size_t i = 0; i < mParticles.size(); ++i ) {
        mParticles[i].p.x += mParticles[i].v.x = mParticles[i].v.x * 0.99f;
        mParticles[i].p.y += mParticles[i].v.y = mParticles[i].v.y * 0.99f - 0.001;
        Blend( &mPool, ULIS2_BLOCKING, ULIS2_PERF_TSPEC | ULIS2_PERF_SSE42, mHost, ULIS2_NOCB, mParticle, mCanvas, sourceRect, mParticles[i].p, ULIS2_AA, BM_SCREEN, AM_NORMAL, 1.f );
    }
