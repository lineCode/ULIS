...
template< typename T >
void
InvokeBlendMTProcessScanline_NonSeparable_CM_Grey_MEM_Generic_Subpixel( int32               iLINE
                                                                      , const tByte*        iSRC
                                                                      , tByte*              iBDP
                                                                      , uint8*              iXIDT
                                                                      , uint8               iBPC
                                                                      ...
                                                                      , ufloat              iOpacity )
{
    ...
    for( int x = 0; x < iBdpROI.w; ++x ) {
        ...
        // Compose
        float subpixel_src_greyf = SampleSubpixelChannelPremult< T >( src, iXIDT[0], iBPP, iSRC_BPS, x, iLINE, 0000, 0000, iSrcROI.w, iSrcROI.h, sub, bus, m11, m01, m10, m00, res );
        float bdp_greyf = TYPE2FLOAT( bdp, iXIDT[0] );
        #define TMP_ASSIGN( _BM, _E1, _E2, _E3 ) FLOAT2TYPE( bdp, iXIDT[0], ComposeF( subpixel_src_greyf, bdp_greyf, alpha_bdp, var, NonSeparableOpF< _BM >( subpixel_src_greyf, bdp_greyf ) ) );
...
template< typename T >
void
BlendMT_NonSeparable_CM_Grey_MEM_Generic_Subpixel( const FFormatInfo& iFormatInfo, const FPerfInfo& iPerfParams, std::shared_ptr< const FBlendInfo > iBlendParams ) {
    /*
    uint8* xidt;
    ...
    ULIS2_MACRO_INLINE_PARALLEL_FOR( iPerf, iPool, iBlocking, iBdpROI.h, InvokeBlendMTProcessScanline_NonSeparable_CM_Grey_MEM_Generic_Subpixel< T >, pLINE
...
template< typename T >
void
InvokeBlendMTProcessScanline_NonSeparable_CM_Grey_MEM_Generic( int32            iLINE
                                                             , const tByte*     iSRC
                                                             , tByte*           iBDP
                                                             , uint8*           iXIDT
                                                             , uint8            iBPC
                                                             ...
                                                             , ufloat           iOpacity )
{
    ...
    for( int x = 0; x < iBdpROI.w; ++x ) {
        ...
        // Compose
        float src_greyf = TYPE2FLOAT( src, iXIDT[0] );
        float bdp_greyf = TYPE2FLOAT( bdp, iXIDT[0] );
        #define TMP_ASSIGN( _BM, _E1, _E2, _E3 ) FLOAT2TYPE( bdp, iXIDT[0], ComposeF( src_greyf, bdp_greyf, alpha_bdp, var, NonSeparableOpF< _BM >( src_greyf, bdp_greyf ) ) );
        ULIS2_SWITCH_FOR_ALL_DO( iBlendingMode, ULIS2_FOR_ALL_NONSEPARABLE_BM_DO, TMP_ASSIGN, 0, 0, 0 )
        #undef TMP_ASSIGN
...
template< typename T >
void
BlendMT_NonSeparable_CM_Grey_MEM_Generic( const FFormatInfo& iFormatInfo, const FPerfInfo& iPerfParams, std::shared_ptr< const FBlendInfo > iBlendParams ) {
/*
    uint8* xidt;
    uint8 bpc, ncc, hea, spp, bpp, aid;
    tSize roi_w, roi_h, src_bps, bdp_bps, src_jmp, bdp_jmp;
    ...
    const tByte*    src = iSource->DataPtr();
    tByte*          bdp = iBackdrop->DataPtr();
    ULIS2_MACRO_INLINE_PARALLEL_FOR( iPerf, iPool, iBlocking, iBdpROI.h, InvokeBlendMTProcessScanline_NonSeparable_CM_Grey_MEM_Generic< T >, pLINE
                                                                       , src + ( ( iSrcROI.y + pLINE ) * src_bps ) + ( iSrcROI.x * bpp )
                                                                       , bdp + ( ( iBdpROI.y + pLINE ) * bdp_bps ) + ( iBdpROI.x * bpp )
                                                                       , xidt, bpc, ncc, hea, spp, bpp, aid, src_bps, iSrcROI, iBdpROI
                                                                       , iSubpixelComponent, iBlendingMode, iAlphaMode, iOpacity );
