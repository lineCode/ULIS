...
template< typename T >
void
InvokeBlendMTProcessScanline_NonSeparable_CM_CMYK_MEM_Generic_Subpixel( const tByte* iSrc, tByte* iBdp, int32 iLine, const tSize iSrcBps, const FFormatInfo* iFmtInfo, std::shared_ptr< const FBlendInfo > iBlendParams ) {
    ...
    for( int x = 0; x < blendInfo._backdropWorkingRect.w; ++x ) {
        ...
        float s11, s01, s10, s00, v1, v2, subpixel_C, subpixel_M, subpixel_Y, subpixel_K;
        SampleSubpixelChannel( subpixel_C, iFmtInfo->IDT[0] );
        SampleSubpixelChannel( subpixel_M, iFmtInfo->IDT[1] );
        SampleSubpixelChannel( subpixel_Y, iFmtInfo->IDT[2] );
        SampleSubpixelChannel( subpixel_K, iFmtInfo->IDT[3] );
        FCMYKF  src_cmykf = { subpixel_C, subpixel_M, subpixel_Y, subpixel_K };
        FCMYKF  bdp_cmykf = { TYPE2FLOAT( bdp, iFmtInfo->IDT[0] ), TYPE2FLOAT( bdp, iFmtInfo->IDT[1] ), TYPE2FLOAT( bdp, iFmtInfo->IDT[2] ), TYPE2FLOAT( bdp, iFmtInfo->IDT[3] ) };
        ...
        for( uint8 j = 0; j < iFmtInfo->NCC; ++j )
            FLOAT2TYPE( bdp, iFmtInfo->IDT[j], ComposeF( *( &(src_cmykf.C) + j ), *( &(bdp_cmykf.C) + j ), alpha_bdp, var, *( &(result_cmykf.C) + j ) ) );
...
BlendMT_NonSeparable_CM_CMYK_MEM_Generic_Subpixel( const FFormatInfo& iFormatInfo, std::shared_ptr< const FBlendInfo > iBlendParams ) {
    ...
    ULIS2_MACRO_INLINE_PARALLEL_FOR( blendInfo.perfInfo.intent, blendInfo.perfInfo.pool, blendInfo.perfInfo.blocking
                                   ...
                                   , InvokeBlendMTProcessScanline_NonSeparable_CM_CMYK_MEM_Generic_Subpixel< T >
...
template< typename T >
void
InvokeBlendMTProcessScanline_NonSeparable_CM_CMYK_MEM_Generic( const tByte* iSrc, tByte* iBdp, int32 iLine, const FFormatInfo* iFmtInfo, std::shared_ptr< const FBlendInfo > iBlendParams ) {
    const FBlendInfo&   blendInfo   = *iBlendParams;
    const tByte*        src         = iSrc;
    tByte*              bdp         = iBdp;
    for( int x = 0; x < blendInfo._backdropWorkingRect.w; ++x ) {
        ...
        FCMYKF  src_cmykf = { TYPE2FLOAT( src, iFmtInfo->IDT[0] ), TYPE2FLOAT( src, iFmtInfo->IDT[1] ), TYPE2FLOAT( src, iFmtInfo->IDT[2] ), TYPE2FLOAT( src, iFmtInfo->IDT[3] ) };
        FCMYKF  bdp_cmykf = { TYPE2FLOAT( bdp, iFmtInfo->IDT[0] ), TYPE2FLOAT( bdp, iFmtInfo->IDT[1] ), TYPE2FLOAT( bdp, iFmtInfo->IDT[2] ), TYPE2FLOAT( bdp, iFmtInfo->IDT[3] ) };
        FRGBF   src_rgbf = CMYKToRGB( src_cmykf );
        FRGBF   bdp_rgbf = CMYKToRGB( bdp_cmykf );
        FRGBF   result_rgbf;
