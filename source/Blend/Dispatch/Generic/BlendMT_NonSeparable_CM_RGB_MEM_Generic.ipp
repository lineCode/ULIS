...
*__________________
*
* @file         BlendMT_NonSeparable_CM_RGB_MEM_Generic.ipp
...
template< typename T >
void
InvokeBlendMTProcessScanline_NonSeparable_CM_RGB_MEM_Generic_Subpixel( const tByte* iSrc, tByte* iBdp, int32 iLine, const tSize iSrcBps, const FFormatInfo* iFmtInfo, std::shared_ptr< const FBlendInfo > iBlendParams ) {
    ...
    for( int x = 0; x < blendInfo._backdropWorkingRect.w; ++x ) {
        ...
        float s11, s01, s10, s00, v1, v2, subpixel_R, subpixel_G, subpixel_B;
        SampleSubpixelChannel( subpixel_R, iFmtInfo->IDT[0] );
        SampleSubpixelChannel( subpixel_G, iFmtInfo->IDT[1] );
        ...
        for( uint8 j = 0; j < iFmtInfo->NCC; ++j )
            FLOAT2TYPE( bdp, iFmtInfo->IDT[j], ComposeF( *( &(src_rgbf.R) + j ), *( &(bdp_rgbf.R) + j ), alpha_bdp, var, *( &(result_rgbf.R) + j ) ) );
...
template< typename T >
void
BlendMT_NonSeparable_CM_RGB_MEM_Generic_Subpixel( const FFormatInfo& iFormatInfo, std::shared_ptr< const FBlendInfo > iBlendParams ) {
    const FBlendInfo&   blendInfo   = *iBlendParams;
    ...
    ULIS2_MACRO_INLINE_PARALLEL_FOR( blendInfo.perfInfo.intent, blendInfo.perfInfo.pool, blendInfo.perfInfo.blocking
                                   ...
                                   , InvokeBlendMTProcessScanline_NonSeparable_CM_RGB_MEM_Generic_Subpixel< T >
...
template< typename T >
void
InvokeBlendMTProcessScanline_NonSeparable_CM_RGB_MEM_Generic( const tByte* iSrc, tByte* iBdp, int32 iLine, const FFormatInfo* iFmtInfo, std::shared_ptr< const FBlendInfo > iBlendParams ) {
    const FBlendInfo&   blendInfo   = *iBlendParams;
    const tByte*        src         = iSrc;
    tByte*              bdp         = iBdp;
    for( int x = 0; x < blendInfo._backdropWorkingRect.w; ++x ) {
...
template< typename T >
void
BlendMT_NonSeparable_CM_RGB_MEM_Generic( const FFormatInfo& iFormatInfo, std::shared_ptr< const FBlendInfo > iBlendParams ) {
    const FBlendInfo&   blendInfo   = *iBlendParams;
    const tByte*        src         = iBlendParams->source->DataPtr();
    tByte*              bdp         = iBlendParams->backdrop->DataPtr();
    const tSize         src_bps     = iBlendParams->source->BytesPerScanLine();
    ...
    ULIS2_MACRO_INLINE_PARALLEL_FOR( blendInfo.perfInfo.intent, blendInfo.perfInfo.pool, blendInfo.perfInfo.blocking
                                   , blendInfo._backdropWorkingRect.h
                                   , InvokeBlendMTProcessScanline_NonSeparable_CM_RGB_MEM_Generic< T >
